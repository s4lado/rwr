...
# Flaga sprawdzająca czy właśnie celujemy
...
func _ready() -> void:
	...
	# Podpinamy sygnały wciśnięcia i puszczenia
	button_pressed.connect(_on_button_pressed)
	button_released.connect(_on_button_released)

func _process(_delta: float) -> void:
	# Pokazuj marker TYLKO jeśli celujemy (trzymamy przycisk) I laser w coś trafia
...
func _on_button_pressed(input_name: String) -> void:
	if input_name == "trigger_click":
		is_aiming = true # Zaczynamy celowanie

func _on_button_released(input_name: String) -> void:
	if input_name == "trigger_click":
		if is_aiming: # Jeśli celowaliśmy, to teraz skaczemy
			teleport_now()
		is_aiming = false # Koniec celowania

func teleport_now() -> void:
	if not ray.is_colliding():
		return
	
	# ... reszta twojej logiki teleportacji bez zmian ...
	...
	# Ważne: usuwamy komponent Y (wysokość) z offsetu kamery, 
	# żeby nie wpaść pod podłogę jeśli gracz kuca
	cam_offset.y = 0 
